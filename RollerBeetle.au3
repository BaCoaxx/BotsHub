...
Func CheckOutpost() ;Check if player is in the race's outpost, then starts the race.
	...
	ElseIf (GetMapID() == 467) Then
		If (GetAgentExists(-2)) Then
            ...
            Do
                Sleep(200)
            Until (CheckAreaWRange(-6367, -4438, 100) OR CheckAreaWRange(-6625,-4435, 100) OR CheckAreaWRange(-6151,-4489, 100) OR CheckAreaWRange(-5936, -4485, 100) OR CheckAreaWRange(-5720, -4483, 100) OR CheckAreaWRange(-5495,-4436, 100)) And GetAgentExists(-2)
		Else
...
;Until CheckAreaWRange(-5729, -4063)
...
Func ClickEnterMission()
    Local $hWnd = WinGetHandle("[CLASS:ArenaNet_Dx_Window_Class]")  
    If $hWnd = 0 Then
        logFile("Error: Could not find Guild Wars window!" & @CRLF)
        Return False
    EndIf

    Local $pos = WinGetPos($hWnd)
    If Not IsArray($pos) Then
        logFile("Error: Could not retrieve window position!" & @CRLF)
        Return False
    EndIf
    ...
    Local $startX = $winX + $winW - 50  
    Local $startY = $winY + ($winH / 2) 

    Local $targetX = $winX + 700 
    Local $targetY = $winY + 450 

    logFile("Starting mouse movement from (" & $startX & ", " & $startY & ") to (" & $targetX & ", " & $targetY & ")" & @CRLF)

    SmoothMouseMove($startX, $startY, $targetX, $targetY, 20)
    MouseClick("left", $targetX, $targetY, 1, 50) 

    logFile("Mission button clicked!" & @CRLF)
    Return True
EndFunc

Func SmoothMouseMove($startX, $startY, $endX, $endY, $steps = 20)
    Local $dx = ($endX - $startX) / $steps
    Local $dy = ($endY - $startY) / $steps

    For $i = 1 To $steps
        MouseMove($startX + ($dx * $i), $startY + ($dy * $i), 5)
        Sleep(10) 
    Next
EndFunc

Func CheckAreWRange($aX, $aY, $range)
    $ret = False
